...
cisco_username = config['cisco_username']
cisco_password = config['cisco_password']

bomgar_username = config['bomgar_username']
bomgar_password = config['bomgar_password']
...
async def scrape_data(page1, page2, page3):
    while True:
        ...
        for i in range(0, 62):
            element = soup.select_one('#dgrid_0-row-' + str(i))
            if element and 'style' in element.attrs:
                row_style = element['style']
